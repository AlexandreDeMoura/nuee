...
const secondaryButton =
  `inline-flex min-h-10.75 cursor-pointer items-center justify-center gap-2.5 rounded-[11px] border border-[#cdd8ea] bg-white px-4.25 py-2.5 text-[14.5px] font-semibold text-[#3f63a8] hover:border-[#aebed8] hover:bg-[#f6f8fc] ${focusRing}`;
const primaryButton =
  `inline-flex min-h-12 cursor-pointer items-center justify-center gap-2.5 rounded-[12px] bg-[#3f63a8] px-4.75 py-3 text-[14.5px] font-semibold text-white hover:bg-[#33538f] ${focusRing}`;
...
function PendingSourceList({
  ...
  return (
    <ul
      className="mt-4.75 space-y-2.5 text-left"
    ...
    >
      {controller.pendingSources.map((source) => {
        ...
        return (
          <li
            className={`flex items-center gap-3.5 rounded-[12px] border bg-white px-3.5 py-3 ${
              issue
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[14.5px] font-semibold text-[#344050]">
                {source.title}
              </span>
              <span className="mt-0.5 block text-[11.5px] font-medium tracking-[0.08em] text-[#8b97a6] uppercase [font-family:'IBM_Plex_Mono',ui-monospace,monospace]">
                {source.kind}
              </span>
              {issue && (
                <span
                  className="mt-1.25 block text-[12.5px] leading-[1.4] text-[#a05b55]"
                  id={issueId}
                >
            ...
            {issue && (
              <span className="shrink-0 rounded-[6px] bg-[#f9eeee] px-1.75 py-1.25 text-[9.5px] font-semibold tracking-[0.07em] text-[#a95f57] [font-family:'IBM_Plex_Mono',ui-monospace,monospace]">
                REVIEW
              </span>
            )}
            <button
              className={`grid size-9.5 shrink-0 cursor-pointer place-items-center rounded-[10px] text-[#9aa6b4] hover:bg-[#fbf1f0] hover:text-[#b4544e] ${focusRing}`}
              type="button"
...
function ProjectContextIndicator() {
  return (
    <div
      className="flex items-center gap-3.5 rounded-[13px] border border-[#cad7ec] bg-[#f4f7fc] px-4.25 py-3.5 text-left"
      aria-label="Project description, always included"
    >
      <span className="grid size-9.5 shrink-0 place-items-center rounded-[10px] bg-[#e5ecf8] text-[#3f63a8]">
        <CircleDot
          className="size-[18px]"
          strokeWidth={1.8}
          aria-hidden="true"
      ...
      <span className="min-w-0">
        <span className="block text-[14.5px] font-semibold text-[#344050]">
          Project description
        </span>
        <span className="mt-0.5 block text-[12.5px] leading-[1.4] text-[#738196]">
          Always included from its latest value when the discussion starts.
        </span>
      </span>
      <span className="ml-auto shrink-0 rounded-[6px] bg-white/80 px-1.75 py-1.25 text-[10px] font-semibold tracking-[0.08em] text-[#6681b5] [font-family:'IBM_Plex_Mono',ui-monospace,monospace]">
        INCLUDED
      </span>
    </div>
  );
...
export function DiscussionContextSelection({
  ...
  if (controller.phase === 'error') {
    return (
      <section
        className="m-auto w-full max-w-[624px] py-6 text-center"
        aria-labelledby="discussion-context-error-title"
      >
        <span className="mx-auto mb-3.5 grid size-13.25 place-items-center rounded-[15px] bg-[#f9eeee] text-[#a95f57]">
          <CircleAlert
            className="size-[22px]"
            strokeWidth={1.7}
            aria-hidden="true"
        ...
        <h3
          className="text-[18px] font-semibold text-[#344050]"
          id="discussion-context-error-title"
        >
          Discussion context wasn&apos;t created
        </h3>
        <p
          className="mx-auto mt-2.5 max-w-[504px] text-[14.5px] leading-[1.55] text-[#8b5d59]"
          role="alert"
        >
          {controller.error ?? 'The discussion could not be started.'}
        </p>
        {controller.pendingSources.length > 0 && (
          <div className="mx-auto mt-4.75 max-w-[552px]">
  ...
  if (controller.phase === 'review') {
    ...
    return (
      <section
        className="m-auto w-full max-w-[672px] py-2.5"
        aria-labelledby="discussion-context-review-title"
      >
        <div className="text-center">
          <p className="text-[11.5px] font-semibold tracking-[0.12em] text-[#8290a0] uppercase [font-family:'IBM_Plex_Mono',ui-monospace,monospace]">
            Frozen when discussion starts
          </p>
          <h3
            className="mt-1.75 text-[20.5px] font-semibold text-[#263344]"
            id="discussion-context-review-title"
          >
            Review discussion context
          </h3>
          <p className="mt-1.75 text-[14.5px] leading-[1.5] text-[#7d8998]">
        ...
        <div className="mt-6">
          {controller.failure && (
            <p
              className="mb-3.5 rounded-[11px] border border-[#e7c5c1] bg-[#fff9f8] px-3.5 py-3 text-left text-[13px] leading-[1.5] text-[#8b5d59]"
              role="alert"
            >
  ...
  return (
    <section
      className="m-auto w-full max-w-[648px] py-3.5 text-center"
    ...
    >
      ...
      <p className="mx-auto mt-2.5 max-w-[540px] text-[14.5px] leading-[1.55] text-[#7d8998]">
      ...
      <button
        className={`${primaryButton} mt-4.75 w-full`}
        type="button"
        onClick={() => submit(true)}
      >
      ...
      <button
        className={`mt-3.5 cursor-pointer rounded-md px-2.5 py-1.25 text-[13px] font-medium text-[#7d8998] hover:text-[#40516a] ${focusRing}`}
        type="button"
        onClick={controller.cancel}
      >
        Back to prompt
