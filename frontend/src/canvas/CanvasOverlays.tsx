...
export function TerritoryPositionSaveError({
  onRetry,
  onRevert,
  territoryTitle,
}: {
  ...
  return (
    <div
      className="pointer-events-auto absolute top-4 left-4 flex max-w-[420px] items-center gap-3 rounded-[10px] border border-[#ead5d2] bg-white/95 px-3.5 py-2.5 text-xs text-[#79504c] shadow-[0_8px_24px_-14px_rgba(30,39,51,0.4)] backdrop-blur-sm"
      data-canvas-overlay
      role="alert"
    >
      <CircleAlert
        ...
        aria-hidden="true"
      />
      <span>Couldn’t save “{territoryTitle}” position.</span>
      <button
...
export function CompactTerritoryLayoutSaveError({
  onRetry,
}: {
  onRetry: () => void;
}) {
  return (
    ...
    >
      ...
      />
      <span>
        Couldn’t save the compact layout. The previous layout was restored.
      </span>
      <button
        className={`shrink-0 cursor-pointer font-semibold text-[#8f4843] hover:text-[#6f3531] ${focusRing}`}
        type="button"
