/**
 * Bubbles are rebuilt after territories so SQLite processes their project
 * cascade before the bubble-to-territory RESTRICT relationship. The rebuild
 * also preserves the composite membership foreign key and provenance guards.
 */
export const MANUAL_TERRITORIES_MIGRATION = `
  CREATE TABLE territories_manual (
    id TEXT PRIMARY KEY CHECK (length(trim(id)) > 0),
    project_id TEXT NOT NULL,
    kind TEXT NOT NULL CHECK (kind IN ('manual', 'ungrouped')),
  ...
  INSERT INTO territories_manual (
    id,
    project_id,
    kind,
    title,
  ...
  SELECT
    id,
    project_id,
    CASE kind WHEN 'composed' THEN 'manual' ELSE kind END,
    title,
    position_x,
    position_y,
    visible_count,
  ...
  ALTER TABLE territories_manual RENAME TO territories;
  ...
  CREATE TABLE bubbles_with_manual_territories (
    id TEXT PRIMARY KEY,
    project_id TEXT NOT NULL,
  ...
  INSERT INTO bubbles_with_manual_territories (
    id,
    project_id,
    territory_id,
    title,
  ...
  DROP TABLE bubbles;
  ALTER TABLE bubbles_with_manual_territories RENAME TO bubbles;
